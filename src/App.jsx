...
function PageRouter({page,onNavigate}){
 const map={dashboard:<Dashboard onNavigate={onNavigate}/>,students:<Students onNavigate={onNavigate}/>,trainers:<Trainers/>,assessments:<Assessments/>,evolution:<Evolution/>,plans:<Plans/>,nutrition:<Nutrition/>,goals:<Goals/>,challenges:<Challenges/>,exercises:<Exercises/>,messages:<Messages/>,reports:<Reports/>,settings:<SettingsPage/>,profile:<Profile onNavigate={onNavigate}/>};
...
function Assessments(){
 ...
 return <><Heading title="Avaliações físicas" sub="Anamnese, perimetria, dobras cutâneas, TANITA, postura e evolução fotográfica."/><Card className="pad section moduleRoadmap"><Activity size={34}/><div><h2>{student?'Ainda não existem avaliações publicadas':'Módulo preparado para o Update 5B'}</h2><p>{student?'Quando o professor publicar uma avaliação, o histórico e os gráficos aparecerão aqui.':'A base real de alunos já está ligada. A próxima etapa acrescentará avaliações modulares, rascunhos, publicação e gráficos sem valores artificiais.'}</p></div></Card></>;
}
