...
import {
  ...
  KeyRound, Save, ShieldCheck, UserRound, Edit3, X,
} from 'lucide-react';
...
export default function ProfessionalProfile() {
  ...
  return <div className="professionalProfilePage">
    ...
    <div className="grid two professionalProfileGrid">
      {!editingProfile ? <section className="card pad professionalReadCard">
        <div className="professionalReadHeader">
          ...
          <button className="secondary" onClick={() => { setNotice(''); setError(''); setEditingProfile(true); }}><Edit3 size={16}/>Editar perfil</button>
        </div>
        <div className="professionalReadPhoto">
          <div className="professionalReadPhotoAvatar">{profile.photoUrl || profile.thumbUrl ? <img src={profile.photoUrl || profile.thumbUrl} alt={profile.full_name}/> : initials(profile.full_name)}</div>
        ...
        <div className="professionalReadGrid">
          ...
          <ReadField label="Rede social / Instagram" wide>{profile.socialUrl ? <a href={profile.socialUrl} target="_blank" rel="noreferrer"><ExternalLink size={14}/>{socialDisplay(profile.socialUrl)}</a> : <b>—</b>}</ReadField>
          <ReadField label="Apresentação profissional" value={profile.biography} wide/>
        </div>
      </section> : <form className="card pad professionalForm" onSubmit={saveProfile}>
        <div className="professionalEditHeader"><div className="panelTitle"><div><h2>Editar dados profissionais</h2><p>Altera apenas os dados que pretendes atualizar.</p></div><UserRound size={24}/></div><button type="button" className="iconButton" onClick={() => { setEditingProfile(false); setPhoto(null); }} title="Cancelar edição"><X/></button></div>
        ...
        <div className="professionalEditActions"><button type="button" className="secondary" onClick={() => { setEditingProfile(false); setPhoto(null); }}>Cancelar</button><button className="primary profileSaveButton" disabled={saving}><Save size={17}/>{saving ? 'A guardar…' : 'Guardar alterações'}</button></div>
      </form>}

      {!editingPassword ? <section className="card pad securityReadCard">
        ...
        <div className="securityReadState"><div className="securityReadIcon"><ShieldCheck/></div><div><b>Palavra-passe configurada</b><p>Por segurança, a palavra-passe atual não é mostrada nesta página. Só precisas de introduzi-la quando quiseres fazer uma alteração.</p></div></div>
      ...
      </section> : <form className="card pad passwordProfileForm" onSubmit={savePassword}>
        <div className="professionalEditHeader"><div className="panelTitle"><div><h2>Alterar palavra-passe</h2><p>Confirma a palavra-passe atual e define uma nova.</p></div><KeyRound size={24}/></div><button type="button" className="iconButton" onClick={() => { setEditingPassword(false); setShowPasswords(false); }} title="Cancelar edição"><X/></button></div>
        ...
        <div className="professionalEditActions"><button type="button" className="secondary" onClick={() => { setEditingPassword(false); setShowPasswords(false); }}>Cancelar</button><button className="primary" disabled={passwordSaving}>{passwordSaving ? 'A alterar…' : 'Guardar nova palavra-passe'}</button></div>
      </form>}
    </div>
  </div>;
}
