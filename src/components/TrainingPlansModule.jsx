...
export default function TrainingPlansModule({ context = {}, onNavigate }) {
  ...
  if (editing) return <PlanEditor initialPlan={editing} students={visibleStudents} exercises={data.exercises} blockTypes={data.blockTypes||[]} onCancel={() => setEditing(null)} onSaved={async id => { setEditing(null); await refreshTraining(); setPlanRoute(id, { replace: true }); }}/>;
  if (activePlan) {
