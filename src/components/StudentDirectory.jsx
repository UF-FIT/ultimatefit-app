...
export function StudentSelfHome({ student, assessments = [], onNavigate, onRefresh }) {
  ...
  const publishedAssessments=[...assessments].filter(item=>item.status==='published').sort((a,b)=>a.date.localeCompare(b.date));
  const latestAssessment=publishedAssessments.at(-1);
  const previousAssessment=publishedAssessments.length>1?publishedAssessments.at(-2):null;
  async function exportLatestAssessment(){if(!latestAssessment){setNotice('Ainda não existe uma avaliação publicada para exportar.');return;}try{setNotice('A preparar relatório PDF…');await downloadAssessmentPdf(student,latestAssessment,previousAssessment);setNotice('Relatório PDF exportado com sucesso.');}catch(err){setNotice(err.message||'Não foi possível exportar o PDF.')}}
