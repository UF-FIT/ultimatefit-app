...
function StudentProfile({ student, currentUser, trainers, assessments, onBack, onEdit, onRefresh, onNavigate }) {
  ...
  async function action(type) {
    const confirmed = type === 'delete'
      ? window.confirm(`Eliminar definitivamente o aluno "${student.name}"?\n\nEsta ação apaga a conta, avaliações, planos, progresso, inscrições, documentos e ficheiros associados. Não pode ser anulada.\n\nSe pretendes apenas retirar o aluno sem perder o histórico, escolhe Arquivar.`)
      : window.confirm(`Confirmas que pretendes ${type === 'archive' ? 'arquivar o aluno' : type === 'deactivate' ? 'desativar o aluno' : 'reativar o aluno'}?`);
    if (!confirmed) return;
  ...
  return <div className="studentProfilePage">
    ...
    <section className="card pad accessPanel profileAccessPanel"><div className="panelTitle"><div><span className="eyebrow">GESTÃO</span><h2>Acesso à aplicação</h2><p>Arquivar preserva o histórico. Eliminar definitivamente apaga todos os dados do aluno.</p></div><Power size={24}/></div><div className="accessStatus"><div><small>ESTADO</small><strong>{studentStatusLabels[student.status] || student.status}</strong><span>{student.invitation?.status === 'pending' ? 'Convite pendente' : student.active ? 'Acesso disponível' : student.status === 'archived' ? 'Arquivado · pode ser reativado' : 'Sem acesso'}</span></div><div className="accessButtons"><button className="secondary" onClick={async()=>{setBusy('resend');try{const r=await invokeStudentAction({action:'resend_access',studentId:student.id});setMessage(r.message)}catch(e){setError(e.message)}finally{setBusy('')}}} disabled={busy==='resend'}><Mail size={16}/>Novo link</button>{student.active ? <button className="secondary" onClick={()=>action('deactivate')} disabled={busy}><Power size={16}/>Desativar</button> : <button className="primary" onClick={()=>action('reactivate')} disabled={busy}><RefreshCw size={16}/>Reativar</button>}{student.status !== 'archived' && <button className="secondary" onClick={()=>action('archive')} disabled={busy}><Archive size={16}/>Arquivar</button>}{canDelete && <button className="dangerButton" onClick={()=>action('delete')} disabled={busy}><Trash2 size={16}/>Eliminar definitivamente</button>}</div></div></section>
...
export default function StudentDirectory({ onNavigate }) {
  ...
  const list = useMemo(() => data.students.filter(student => {
    ...
    const lifecycleStatus = student.status;
    const matchesStatus = status === 'all' || lifecycleStatus === status;
  ...
  async function bulkAction(action) {
    ...
    const confirmed = action === 'delete'
      ? window.confirm(`Eliminar definitivamente ${selected.size} aluno(s)?\n\nTodos os dados associados serão apagados e esta ação não pode ser anulada. Para preservar histórico, usa Arquivar.`)
      : window.confirm(`Aplicar esta ação a ${selected.size} aluno(s)?`);
    if (!confirmed) return;
    setError('');setNotice('');
    try {
