...
import {
  activityLevelLabel, archiveAssessment, assessmentMetrics, assessmentModuleLabels, bmiCategory,
  assessmentStatusLabel, deleteAssessmentPermanently, publishAssessment, riskResultLabel,
...
function NumberField({ name, label, unit, value, onValueChange, hint }) { return <Field label={label}><div className="unitInput"><input name={name} type="number" step="0.01" min="0" defaultValue={value ?? ''} onChange={event=>onValueChange?.(event.target.value)}/>{unit&&<small>{unit}</small>}</div>{hint&&<span className="bmiCategoryLive">{hint}</span>}</Field>; }
...
function BioSection({ values = {} }) { const [bmiValue,setBmiValue]=useState(values.bmi ?? ''); const bmiLabel=bmiCategory(bmiValue); return <section className="assessmentFormSection"><div className="assessmentSectionTitle"><Scale/><div><h2>Bioimpedância · TANITA</h2><p>Campos alinhados com o registo usado no estúdio. A classificação do IMC é apresentada automaticamente.</p></div></div><div className="assessmentNumbersGrid">{bioFields.map(([key,label,unit])=><NumberField key={key} name={`bioimpedance.${key}`} label={label} unit={unit} value={values[key]} onValueChange={key==='bmi'?setBmiValue:undefined} hint={key==='bmi'?bmiLabel:''}/>)}</div><input type="hidden" name="bioimpedance.device" value="TANITA"/><TextArea name="bioimpedance.notes" label="Observações TANITA" value={values.notes}/></section>; }
...
function AssessmentDetail({ assessment, student, onBack, canManage, onEdit, onArchive, onDelete }) {
  ...
  return <div className="assessmentDetail"><button className="backButton" onClick={onBack}><ArrowLeft size={18}/>Voltar ao histórico</button><div className="assessmentDetailHero card"><div><span className="eyebrow">{student.name}</span><h1>Avaliação · {fmt(assessment.date)}</h1><div className="assessmentBadges"><Status status={assessment.status}/>{assessmentModuleLabels(assessment).map(label=><span key={label}>{label}</span>)}</div></div><div className="assessmentDetailActions">{assessment.status==='published'&&<button className="secondary" onClick={()=>downloadAssessmentPdf(student,assessment)}><FileText size={16}/>Exportar avaliação em PDF</button>}{canManage&&<><button className="secondary" onClick={onEdit}><Edit3 size={16}/>Editar</button>{assessment.status==='published'&&<button className="secondary" onClick={onArchive}><Archive size={16}/>Arquivar</button>}<button className="secondary destructiveButton" onClick={onDelete}><Trash2 size={16}/>Eliminar definitivamente</button></>}</div></div>
    ...
    {modules.bioimpedance&&<section className="card pad assessmentReadSection"><h2>Bioimpedância · TANITA</h2><div className="assessmentReadGrid">{bioFields.map(([key,label,unit])=>{const value=modules.bioimpedance[key];return <div key={key}><small>{label}</small><b>{value??'—'} {value!=null?unit:''}</b>{key==='bmi'&&bmiCategory(value)&&<span className="bmiCategoryBadge">{bmiCategory(value)}</span>}</div>})}</div></section>}
...
export default function AssessmentsModule({ context = {}, onNavigate }) {
  ...
  if(viewing&&selectedStudent) return <AssessmentDetail assessment={viewing} student={selectedStudent} canManage={canManage} onBack={()=>setViewing(null)} onEdit={()=>{setEditing(viewing);setViewing(null);setMode('form')}} onArchive={()=>archive(viewing)} onDelete={()=>removeAssessment(viewing)}/>;
