...
import {
  ...
  assessmentModuleLabels, assessmentReferences, automaticRiskSummary, bioimpedanceIndicator, bodyFatCategory,
...
function BioSection({ values = {}, student, assessmentDate }) {
  ...
  return <section className="assessmentFormSection">
    ...
    <div className="assessmentNumbersGrid">{bioFields.map(([key,label,unit])=>{
      const hint=bioimpedanceIndicator(key,draft[key],draft,student,assessmentDate);
      return <NumberField key={key} name={`bioimpedance.${key}`} label={label} unit={unit} value={values[key]} onValueChange={value=>updateValue(key,value)} hint={hint}/>;
    })}</div>
    {(draft.weight_kg||draft.water_pct||draft.visceral_fat_rating||draft.bone_mass_kg||draft.muscle_mass_kg||draft.body_fat_pct)&&<div className="assessmentReferenceNote">
      ...
      <span>Massa muscular: a TANITA classifica a massa relativa à altura através de um Muscle Score próprio. Como o valor em kg não permite reconstruir esse score com rigor, a APP mostra o índice muscular, mas não inventa uma categoria baixo/médio/alto.</span>
...
function AssessmentDetail({ assessment, student, previousAssessment, onBack, canManage, onEdit, onArchive, onDelete, onExport }) {
  ...
  return <div className="assessmentDetail"><button className="backButton" onClick={onBack}><ArrowLeft size={18}/>Voltar ao histórico</button><div className="assessmentDetailHero card"><div><span className="eyebrow">{student.name}</span><h1>Avaliação · {fmt(assessment.date)}</h1><div className="assessmentBadges"><Status status={assessment.status}/>{assessmentModuleLabels(assessment).map(label=><span key={label}>{label}</span>)}</div></div><div className="assessmentDetailActions">{assessment.status==='published'&&<button className="secondary" onClick={onExport}><FileText size={16}/>Exportar avaliação em PDF</button>}{canManage&&<><button className="secondary" onClick={onEdit}><Edit3 size={16}/>Editar</button>{assessment.status==='published'&&<button className="secondary" onClick={onArchive}><Archive size={16}/>Arquivar</button>}<button className="secondary destructiveButton" onClick={onDelete}><Trash2 size={16}/>Eliminar definitivamente</button></>}</div></div>
    ...
    {modules.bioimpedance&&<section className="card pad assessmentReadSection"><h2>Bioimpedância · TANITA</h2><div className="assessmentReadGrid">{bioFields.map(([key,label,unit])=>{const value=modules.bioimpedance[key];const category=bioimpedanceIndicator(key,value,modules.bioimpedance,student,assessment.date);return <div key={key}><small>{label}</small><b>{value??'—'} {value!=null?unit:''}</b>{category&&<span className="bmiCategoryBadge">{category}</span>}</div>})}</div><div className="assessmentBibliography"><b>Referências de bioimpedância</b><span>{assessmentReferences.weight} {assessmentReferences.water} {assessmentReferences.visceral} {assessmentReferences.bone}</span><span>{assessmentReferences.muscle}</span><span>{assessmentReferences.biaCaution}</span>{fatClass&&<span>{assessmentReferences.bodyFat}</span>}</div></section>}
