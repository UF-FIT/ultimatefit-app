...
import {
  ...
  assessmentModuleLabels, assessmentReferences, automaticRiskSummary, bmiCategory, bodyFatCategory,
...
function BioSection({ values = {}, student, assessmentDate }) { const [bmiValue,setBmiValue]=useState(values.bmi ?? ''); const [fatValue,setFatValue]=useState(values.body_fat_pct ?? ''); const bmiLabel=bmiCategory(bmiValue); const age=ageAtAssessment(student?.birth,assessmentDate); const fatLabel=bodyFatCategory(fatValue,age,student?.sex); return <section className="assessmentFormSection"><div className="assessmentSectionTitle"><Scale/><div><h2>Bioimpedância · TANITA</h2><p>IMC e percentual de gordura recebem classificação automática sempre que existam dados de referência aplicáveis.</p></div></div><div className="assessmentNumbersGrid">{bioFields.map(([key,label,unit])=><NumberField key={key} name={`bioimpedance.${key}`} label={label} unit={unit} value={values[key]} onValueChange={key==='bmi'?setBmiValue:key==='body_fat_pct'?setFatValue:undefined} hint={key==='bmi'?bmiLabel:key==='body_fat_pct'?fatLabel:''}/>)}</div>{fatValue&&<div className="assessmentReferenceNote"><b>Percentual de gordura: {fatLabel || 'sem classificação de referência'}</b><span>{fatLabel?`Referência para ${age} anos e sexo registado no perfil. `:'A classificação Gallagher aplica-se a adultos dos 20 aos 79 anos e requer sexo masculino/feminino registado no perfil. '}{assessmentReferences.bodyFat}</span></div>}<input type="hidden" name="bioimpedance.device" value="TANITA"/><TextArea name="bioimpedance.notes" label="Observações TANITA" value={values.notes}/></section>; }
...
function AssessmentDetail({ assessment, student, previousAssessment, onBack, canManage, onEdit, onArchive, onDelete, onExport }) {
  ...
  return <div className="assessmentDetail"><button className="backButton" onClick={onBack}><ArrowLeft size={18}/>Voltar ao histórico</button><div className="assessmentDetailHero card"><div><span className="eyebrow">{student.name}</span><h1>Avaliação · {fmt(assessment.date)}</h1><div className="assessmentBadges"><Status status={assessment.status}/>{assessmentModuleLabels(assessment).map(label=><span key={label}>{label}</span>)}</div></div><div className="assessmentDetailActions">{assessment.status==='published'&&<button className="secondary" onClick={onExport}><FileText size={16}/>Exportar avaliação em PDF</button>}{canManage&&<><button className="secondary" onClick={onEdit}><Edit3 size={16}/>Editar</button>{assessment.status==='published'&&<button className="secondary" onClick={onArchive}><Archive size={16}/>Arquivar</button>}<button className="secondary destructiveButton" onClick={onDelete}><Trash2 size={16}/>Eliminar definitivamente</button></>}</div></div>
    ...
    {modules.bioimpedance&&<section className="card pad assessmentReadSection"><h2>Bioimpedância · TANITA</h2><div className="assessmentReadGrid">{bioFields.map(([key,label,unit])=>{const value=modules.bioimpedance[key];const category=key==='bmi'?bmiCategory(value):key==='body_fat_pct'?bodyFatCategory(value,assessmentAge,student?.sex):'';return <div key={key}><small>{label}</small><b>{value??'—'} {value!=null?unit:''}</b>{category&&<span className="bmiCategoryBadge">{category}</span>}</div>})}</div>{fatClass&&<div className="assessmentBibliography"><b>Percentual de gordura</b><span>{assessmentReferences.bodyFat}</span></div>}</section>}
