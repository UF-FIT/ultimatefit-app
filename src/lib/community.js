...
export function mapNotice(row){return {id:row.id,title:row.title,body:row.body,targetAudience:row.target_audience,showPopup:row.show_popup!==false,showDashboard:row.show_dashboard!==false,active:row.is_active!==false,activeFrom:row.active_from,activeUntil:row.active_until,createdAt:row.created_at};}
...
export async function saveNotice(input){
  const payload={title:input.title.trim(),body:input.body.trim(),target_audience:input.targetAudience||'students',show_popup:input.showPopup!==false,show_dashboard:input.showDashboard!==false,is_active:input.active!==false,active_from:input.activeFrom||new Date().toISOString(),active_until:input.activeUntil||null};
...
export async function deleteNotice(id){const {error}=await supabase.from('app_notices').delete().eq('id',id);if(error)throw error;}

export function mapActivity(row){return {id:row.id,title:row.title,slug:row.slug,description:row.description||'',eventDate:row.event_date,startTime:row.start_time||'',location:row.location||'',feeCents:row.fee_cents||0,capacity:row.capacity,posterUrl:row.poster_url||'',registrationOpen:row.registration_open!==false,registrationDeadline:row.registration_deadline||'',active:row.is_active!==false,createdAt:row.created_at};}
